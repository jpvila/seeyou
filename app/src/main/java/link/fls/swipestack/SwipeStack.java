...
import link.fls.swipestacksample.AdministradorRespuestaSingleton;
...
//            ImageView fotoPersona = (ImageView) bottomView.findViewById(R.id.imag_foto_persona);
//            fotoPersona.setImageResource(R.drawable.tostado_alta_definicion);
...
/*
    public void verVista(){
        if (mTopView != null) {
            View v = ((ViewGroup) mTopView).getChildAt(0);
            if(v!= null){
                Bitmap bitmap = BitmapFactory.decodeResource(getResources(), R.drawable.playadelcarmen);
                Bitmap blurredBitmap = blur(bitmap);
                ((ImageView) v).setImageBitmap(blurredBitmap);
            }
        }
...
*/
    public void acertoRespuesta(){
        if (mTopView != null) {
            View v = ((ViewGroup) mTopView).getChildAt(0);
            if(v!= null){
                Bitmap blurredBitmap = null;
                if( BDSingleton.getInstance().getPersonaActual().getCountOk() == 1){
                    Bitmap bitmap = BitmapFactory.decodeResource(getResources(), BDSingleton.getInstance().getPersonaActual().getIdResourceImagen());
